    ...
    'theProblem' : [
        {
            ...
            'content' : 'FINCHAIN会致力建立并期望成为全球最大的数字货币和区块链社群聚集地。千里之行始于足下，我们会利用发行的数字货币去激励用户生成有质素的内容、自发参与社群讨论和和投票。我们会与每一个用户(包括内容创造者，投票和留言者)分享平台的成功。根据贡献度的多寡，他们能得到相应的FINCHAIN或FINPOWER作为回报。'
        },
        {
            'header' : '数字货币市场上的彭博',
            'content' : 'FINCHAIN 的目标是成为数字货币市场上的彭博，提供最专业的分析。FINCHAIN 分析平台会建构数字货币和区块链的数据库，可为用户提供数字货币信息和业内动态内容，并通过客观数据反馈为用户提供交易建议和顾问服务。数字货币和区块链的价格现时与市场情绪、兴奋度等息息相关，初期会专注于中英文情感分析、文本意见挖掘、多源数据信息推送。用户将可通过手机和网络终端查看多源数据、各种专业图表和独立分析。未来会引入更多的数据信息流和各种的机械学习模型以其为用户带来更精准的洞察、分析和信息上的优势。我们也将开发多种扩展应用程序，以方便用户可直接链接第三方服务，如交易所和资产管理平台。'
        }
    ],
    'solution' : {
        'title' : '关键功能',
